...
const Nu3d = () => {
  ...
  const [initialProducts, setInitialProducts] = useState<Nu3dProduct[]>([]);
    ...
    const fetchInitialProducts = async () => {
      ...
      try {
        ...
        if (response.success && response.products && response.products.length > 0) {
          // Transform the simple product structure to match Nu3dProduct format
          const transformedProducts: Nu3dProduct[] = response.products.map((productImage) => {
        ...
        console.warn("[Nu3d] Failed to fetch initial products from /api/products:", error);
      } finally {
              ...
              <div className="flex items-center gap-3">
                {variantRows.length > 0 && (
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <span>{displayTotal} {t("nu3d.products") || "products"}</span>
                  </div>
                )}
                <Button
